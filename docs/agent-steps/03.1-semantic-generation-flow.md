...
| `npm run semantic:catalog -- --sample FacialHair:1 --run --max-calls 1` | 只调用 1 次 Cloudflare REST 视觉模型，并输出样本报告 |
| `npm run semantic:catalog -- --run --group facial_hair --offset 0 --limit 3 --max-calls 3 --resume` | 按分组小批量补充标注 |
| `npm run semantic:catalog -- --run --all --resume --max-calls 282` | 全量补充标注，必须显式提供调用上限 |
| `npm run semantic:catalog -- --resume` | 复用已有标注和截图缓存 |
| `npm run semantic:check` | 校验已提交 semantic catalog |

安全约束：

- `--run` 默认拒绝无边界全量执行，必须同时提供 `--sample`、`--group`、`--limit`、`--option-id` 或 `--all`。
- `--option-id` 精确选择优先；否则先按 `--group` 过滤，再应用 `--offset` / `--limit`。
- `--max-calls` 是硬上限；选中数量超过上限时，在截图和 API 调用前失败。
- 单样本报告写入 `.cache/avatar-semantic/samples/`，包含截图状态、视觉模型输入 prompt、原始响应和解析结果。
- 每次真实运行写入 `.cache/avatar-semantic/runs/`，记录选择范围、调用数量和 optionId 列表。
...
| Workflow | 作用 |
| --- | --- |
| `Semantic catalog smoke` | 手动运行单样本或小批量标注，上传截图、样本 JSON、run report、catalog 和 review artifact，不提交仓库 |
| `Semantic catalog full labeling` | 手动全量标注，必须输入 `RUN_FULL_SEMANTIC_CATALOG`，完成后创建 Draft PR，不直接合入 `master` |
...
| `tags` | 审核后的语义标签；可能来自开放标注，但进入 catalog 前必须人工确认 |
| `confidence` | 标签置信度 |
| `visible` | 是否可见 |
