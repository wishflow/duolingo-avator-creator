...
- 本地 Codex：在 Codespaces 中生成三栏图和任务包，再由当前 Codex 对话读取任务包并输出开放 JSONL，人工审核后只合并 `approved`。
...
**重要边界：`npm run semantic:codex:tasks` 不会调用 Codex API，也不会自动生成 `labels/*.jsonl`。**

它只负责截图和生成任务包。真正的 Codex 标注发生在当前聊天窗口：你把某个 batch task 交给 Codex，Codex 读取该批图片和任务 JSON 后，才写出对应的 JSONL。
...
| --- | --- | --- | --- |
| Codex smoke 任务包 | `npm run semantic:codex:tasks -- --smoke` | 否 | 截图和 `.cache/avatar-semantic/codex/tasks/smoke-001.json`，不含 labels |
| Codex 全量任务包 | `npm run semantic:codex:tasks -- --all-features --batch-size 10` | 否 | 截图和 `.cache/avatar-semantic/codex/tasks/features-*.json`，不含 labels |
| Codex 对话标注 | 在当前对话中请求 Codex 处理某个 batch | 否 | `.cache/avatar-semantic/codex/labels/<batch>.jsonl` |
...
flowchart TD
  ...
  B --> C[当前对话中的 Codex 读取单个 batch]
  C --> N[Codex 写出开放 v2 JSONL]
  N --> D[Codex batch report / index]
...
本地 Codex 链路分成两步，不能跳过：

| 步骤 | 谁执行 | 结果 |
| --- | --- | --- |
| 生成任务包 | `semantic:codex:tasks` 脚本 | screenshots、task JSON、labels 目标路径 |
| 生成 labels | 当前聊天窗口中的 Codex | `labels/<batch>.jsonl` |

如果 report 里出现 `labeledCount: 0`、`missingCount > 0`、`labels file not found`，表示第一步完成了，但第二步还没做。这不是截图失败，也不是模型标注为空。
...
接着在当前对话里明确要求 Codex 处理这一批。推荐提示词：

```text
请标注 .cache/avatar-semantic/codex/tasks/smoke-001.json。
只读取这个 task 和其中引用的图片。
把结果写入 .cache/avatar-semantic/codex/labels/smoke-001.jsonl。
每个 option 一行 v2 JSONL，reviewStatus 全部保持 pending。
不要读取其他 batch，也不要修改正式 catalog。
```
...
```text
请标注 .cache/avatar-semantic/codex/tasks/features-001.json。
只处理这个 batch。
把 JSONL 写入 .cache/avatar-semantic/codex/labels/features-001.jsonl。
tags 使用开放视觉标签，attributes 写多维特征，reviewStatus 保持 pending。
```

Codex 完成后才会出现对应 labels 文件：
...
这个命令生成的是 18 个左右的 batch 任务包和截图。它不会循环调用 Codex，也不会自动生成 `features-001.jsonl` 到 `features-018.jsonl`。
...
临时看全量效果时，可以显式允许 `pending` 行直接合并：

```bash
npm run semantic:codex:merge -- --all-labels --allow-pending
npm run semantic:check
```

这个模式只用于先落地查看效果，默认门禁不变；`rejected` 行仍会被拒绝合并。
...
- 默认禁止把 Codex `pending` 或 `rejected` 结果合并到 catalog；只有明确要临时看效果时，才允许使用 `--allow-pending`，且 `rejected` 仍禁止合并。
